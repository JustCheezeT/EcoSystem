...
public class InterestHandler {
	...
	public InterestHandler(Main pl) {
		this.pl = pl;
		interestTask();
	}
    ...
    private void interestTask() {
    	if (pl.getConfigurationHandler().getBoolean("general.interest.enabled") == true) {
    		Main.log.info("Task lãi xuất đã bắt đầu. Tiền lãi sẽ được thêm mỗi " + pl.getConfigurationHandler().getInteger("general.interest.interestTime") + " phút.");
    		...
    		Main.log.info("Task lãi xuất đã bị vô hiệu hóa.");
    	}
    }
