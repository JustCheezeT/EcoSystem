...
public class WithdrawCmd {
	...
	public boolean runUserCmd(CommandSender sender, String[] args) {
		if (sender instanceof Player) {
			...
			if (p.hasPermission("EcoSystem.cmd.withdraw")) {
    			...
    			if (bankBalance >= amount) {
    				if (Main.econ.getBalance(p) + amount > Double.parseDouble(m.getConfigurationHandler().getString("general.maxPocketLimitMoney"))) {
	    ...
	    if (sender instanceof Player) {
	      ...
	      if (p.hasPermission("EcoSystem.admin")) {
	        ...
	        if (target != null) {
	          ...
	          p.sendMessage(ChatColor.RED + "Bạn phải là người chơi để thực hiện lệnh!");
	        }
	      } else {
	        m.getSoundHandler().sendPlingSound(p);
	        m.getConfigurationHandler().printMessage(p, "chatMessages.noPermission", "0", p, p.getName());
	      ...
	      if (target != null) {
	        ...
	        sender.sendMessage(ChatColor.RED + "Bạn phải là người chơi để thực hiện lệnh!");
	      }
	    }
	    return true;
	  }
